...
・インストールの自動化をしたいです（複雑なユーザーのエラーをチャットで解決、または試行錯誤したエラーの収集方法）
・重いデータを削除するコマンドをユーザー為に用意。checkpointなど

・現在の複雑なケースに対応できるようにしたい

コードのリファクタリング（他ファイルのドキュメント化）は、必要が生じたタイミング、あるいは「技術的負債返済週間（Refactoring Sprint）」を設けて実施しましょう。